...
ARG BOOTSTRAP
FROM ${BOOTSTRAP:-alpine:3.8} as builder

WORKDIR /gentoo

ARG ARCH=arm
ARG MICROARCH=armv6j_hardfp
ARG DIST=https://ftp-osl.osuosl.org/pub/gentoo/releases/${ARCH}/autobuilds
ARG SIGNING_KEY="0xBB572E0E2D182910"

RUN echo "Building Gentoo Container image for ${MICROARCH} fetching from ${DIST}" \
 && apk --no-cache add gnupg tar wget xz \
 && STAGE3PATH="$(wget -O- "${DIST}/latest-stage3-${MICROARCH}.txt" | tail -n 1 | cut -f 1 -d ' ')" \
