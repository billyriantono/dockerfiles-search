...
WORKDIR ${ROOT_HOME}
RUN git clone --depth=1 https://beagle-lib@github.com/beagle-dev/beagle-lib.git
...
RUN echo 'export PATH=/usr/local/beast/bin:/usr/local/nvidia/bin:/usr/local/cuda/bin:$PATH' >> ${USER_HOME}/.bashrc
RUN echo 'export LD_LIBRARY_PATH=/usr/local/lib:/usr/local/nvidia/lib:/usr/local/nvidia/lib64:${LD_LIBRARY_PATH}' >> ${USER_HOME}/.bashrc
RUN echo 'export LIBRARY_PATH=/usr/local/cuda/lib64/stubs:${LIBRARY_PATH}' >> ${USER_HOME}/.bashrc
