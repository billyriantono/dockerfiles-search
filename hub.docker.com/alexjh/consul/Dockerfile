...
RUN apk add --update ca-certificates curl git make go zip \
  && git clone https://github.com/alexjh/consul --branch compress-dns --single-branch ${CONSUL_DIR} \
  && make -C ${CONSUL_DIR} tools dev \
  && cp ${CONSUL_DIR}/bin/consul /bin/ \
  && mkdir /tmp/ui && cd /tmp/ui \
  && curl https://releases.hashicorp.com/consul/${CONSUL_VERSION}/consul_${CONSUL_VERSION}_web_ui.zip -o /tmp/ui.zip \
  && unzip /tmp/ui.zip \
  && mkdir /ui \
  && mv * /ui \
