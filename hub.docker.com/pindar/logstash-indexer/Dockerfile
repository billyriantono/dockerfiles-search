...
RUN apt-get update -qq && \
	...
	cd /tmp && wget  --no-cookies --header "Cookie: gpw_e24=http%3A%2F%2Fwww.oracle.com; oraclelicense=accept-securebackup-cookie; s_nr=1394181036342;" http://download.oracle.com/otn-pub/java/jdk/${JAVA_VERSION}-b13/jdk-${JAVA_VERSION}-linux-x64.tar.gz && \
...
RUN curl -L -o /tmp/logstash.tar.gz https://download.elasticsearch.org/logstash/logstash/logstash-${LOGSTASH_VERSION}.tar.gz && \
	mkdir -p /logstash && \
