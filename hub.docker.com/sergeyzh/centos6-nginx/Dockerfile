...
RUN rpm --rebuilddb ; yum localinstall -y /root/rpms/*.rpm && \
    ...
    yum install -y pcre-devel libxml2-devel libxslt-devel GeoIP-devel gd-devel perl-ExtUtils-Embed.x86_64 nginx which 
...
RUN cd /usr/src && wget http://nginx.org/download/nginx-$NGINX_VERSION.tar.gz && \
    tar zxf nginx-$NGINX_VERSION.tar.gz && mkdir nginx-$NGINX_VERSION-src-rpm && cd nginx-$NGINX_VERSION-src-rpm && \
    wget http://nginx.org/packages/rhel/6/SRPMS/nginx-$NGINX_VERSION-1.el6.ngx.src.rpm && \
