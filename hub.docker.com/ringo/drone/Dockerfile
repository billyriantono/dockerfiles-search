...
RUN apt-get install -y wget git mercurial bzr gcc make vim && \
    apt-get clean
RUN wget --no-check-certificate -O /var/tmp/go1.2.2.linux-amd64.tar.gz https://storage.googleapis.com/golang/go1.2.2.linux-amd64.tar.gz && \
...
RUN sed -i -e 's/https:\/\/" + settings.GitHubDomain/http:\/\/" + settings.GitHubDomain/g' pkg/handler/auth.go
