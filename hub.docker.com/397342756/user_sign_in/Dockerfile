...
RUN mv composer.phar /usr/local/bin/composer && apt-get update && apt-get install -y libmcrypt-dev git zlib1g-dev && docker-php-ext-install mbstring mcrypt pdo_mysql zip
WORKDIR /tmp/composer-install
ADD ./composer.json composer.json
ADD ./composer.lock composer.lock
RUN composer install --no-dev && sed -i 's#DocumentRoot /var/www/html#DocumentRoot /var/www/webapp/public#' /etc/apache2/apache2.conf
COPY . /var/www/webapp
RUN chown www-data -R /var/www/webapp/storage && cp -r /tmp/composer-install/vendor /var/www/webapp
WORKDIR /var/www/webapp
CMD bash /var/www/webapp/start.sh
