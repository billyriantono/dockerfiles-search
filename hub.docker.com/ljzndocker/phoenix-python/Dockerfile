...
RUN set -ex ;\
    ...
    apk add --no-cache $PACKAGES || \
        (sed -i -e 's/dl-cdn/dl-4/g' /etc/apk/repositories && apk add --no-cache --force-broken-world $PACKAGES)

    # Add packages just for the python build process with a CDN-breakage fallback if neede
RUN apk add --no-cache --virtual .build-deps $PYTHON_BUILD_PACKAGES || \
        (sed -i -e 's/dl-cdn/dl-4/g' /etc/apk/repositories && apk add --no-cache --virtual --force-broken-world .build-deps $PYTHON_BUILD_PACKAGES) ;\
