...
RUN DEBIAN_FRONTEND=noninteractive apt-get update && apt-get --yes install aptitude
RUN aptitude install -y bc bison build-essential curl flex g++-multilib gcc-multilib git gnupg \
...
RUN apt remove openjdk-7-jre-headless pkg-config-arm-linux-gnueabihf pkg-config-arm-linux-gnueabi
...
# Get Sources
RUN git clone https://github.com/koreader/koxtoolchain ~/Git/Github/Ereaders/koxtoolchain
RUN git clone https://github.com/koreader/koreader ~/Git/Github/Ereaders/koreader
