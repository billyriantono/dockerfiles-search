...
LABEL maintainer="yu-ji"

ENV UNIT_VERSION 1.2-1~bionic
ENV PHP_VERSION 7.2.5-*

RUN set -x \
    && apt-get update \
    && DEBIAN_FRONTEND=noninteractive apt-get install --no-install-recommends --no-install-suggests -y gnupg1 apt-transport-https ca-certificates \
    && \
    ...
    && phpPackages="php7.2-mysql=${PHP_VERSION} php7.2-curl=${PHP_VERSION} php7.2-zip=${PHP_VERSION}" \
    && case "$dpkgArch" in \
        amd64|i386) \
