...
WORKDIR /opt/src/rancher-letsencrypt
COPY . .
...
RUN apk update && apk add --no-cache ca-certificates openssl bash && update-ca-certificates
...
COPY --from=builder /opt/src/rancher-letsencrypt/build/rancher-letsencrypt-linux-amd64 /usr/bin/rancher-letsencrypt
